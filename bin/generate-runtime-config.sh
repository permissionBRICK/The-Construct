...
jq -s '
    ...
    );
  {
    ...
    provisionCommands: (
      # One {profile, dir, command} per command; retain profile identity so
      # the runner can parallelize profiles while keeping their commands in order.
      # dir is each profile FIRST repo checkout directory (resolved the same way
      ...
      map(
        ...
        | ((.provisionCommands // [])
           ...
           | map({ profile: $profile, dir: $dir, command: . }))
      )
      | add
    ),
