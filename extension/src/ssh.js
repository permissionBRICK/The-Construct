 ...
 * name can resolve to IPv6 first. With ConnectTimeout set (this argv always sets it),
 * Windows OpenSSH connects non-blocking and misreads the unreachable first address as
 * established -- the refusal only surfaces on the first write -- so it never tries the
 * next address. Empty otherwise, so every other argv is unchanged. Pure.
 */
