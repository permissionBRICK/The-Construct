...
const remote = { name: "vm-1", backend: "hyperv-remote" };
const offer = { host: "pc-1", url: "https://pc-1:7462" };
...
(async () => {
  ...
  assert.ok(messages.filter(m => m.type === "hostAdminOffer").every(m => m.instance === "vm-1" && m.offer === offer));
  ...
  receive({ data: { type: "hostAdminOffer", instance: "vm-1", offer } });
  assert.equal(element("lHostAdmin").hidden, false);
  assert.equal(element("lHostAdmin").title, "Administer pc-1");
  receive({ data: { type: "state", state: { instance: "vm-1", online: false } } });
  ...
  receive({ data: { type: "hostAdminOffer", instance: "vm-1", offer: null } });
