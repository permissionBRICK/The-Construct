...
test("hostnames cannot carry shell syntax or URLs", () => {
  for (const host of ["pc-2", "pc-2.home.test", "192.168.1.2"]) assert.equal(conversion.validHost(host), true);
  for (const host of ["", "https://pc-2", "a;calc", "a$(calc)", "a..b", "a\nb", "-flag"]) assert.equal(conversion.validHost(host), false);
});
test("adoption preserves SSH identity, config branch, other instances, and rejects stale results", () => {
  ...
  const plan = { id: "test", name: "agent-vm", adminUser: "PC\\alice", publicHost: "pc-2", fingerprint: instances.targetFingerprint(original) };
  const result = { ok: true, id: "test", name: "agent-vm", owner: plan.adminUser, publicHost: "pc-2", url: "https://pc-2:7462", sshPort: 2201 };
