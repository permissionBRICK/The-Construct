...
(() => {
  ...
  eq("hostLabel: an IPv4-mapped literal is a literal too",
    f.sanitizeHostLabel("::ffff:203.0.113.1"), "::ffff:203.0.113.1");
  ...
  const IPV6_LABELS_VALID = [
    ...
    "0:0:0:0:0:0:0:0", "1::", "::2", "0::0", "::ffff:203.0.113.1",
  ...
  eq("urlHost: an IPv4 literal too", f.urlHostFor("203.0.113.7"), "203.0.113.7");
...
async function remoteFlow() {
  ...
  });
  {
    ...
    const bad = f.readForwardList([childEntry({ destination: { vmName: "work-vm-a1", connectAddress: "203.0.113.1 -o ProxyCommand=evil" } })]);
  ...
  {
    const read = f.readForwardList([childEntry({ destination: { vmName: "../evil", via: "work-vm", connectAddress: "203.0.113.1" } })]);
...
(() => {
  ...
  eq("ssh -L: an unrecognised bind address falls back to loopback, never to a guess",
    ssh.buildLocalForwardArgs({}, 18800, 5173, false, { bindHost: "203.0.113.5; rm -rf /" })
      .find((a, i, all) => all[i - 1] === "-L"),
  ...
  eq("ssh -L: an address that is not an address is refused, never emitted", (() => {
    try { ssh.buildLocalForwardArgs({}, 18800, 8080, false, { connectAddress: "203.0.113.1 -o ProxyCommand=x" }); return "no throw"; } catch (_) { return "threw"; }
  })(), "threw");
  eq("ssh -L: a bad connect port is refused", (() => {
    try { ssh.buildLocalForwardArgs({}, 18800, 8080, false, { connectAddress: "203.0.113.1", connectPort: 0 }); return "no throw"; } catch (_) { return "threw"; }
  })(), "threw");
