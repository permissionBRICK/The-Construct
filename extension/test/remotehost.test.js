...
(async () => {
  ...
  for (const rows of [[], [{ name: "vm-1" }], [{ name: "one" }, { name: "two" }]]) {
  ...
  ok("http: a LAN IP is refused too", /Refusing to talk/.test(buildable("http://10.200.0.5:7462")));
  eq("http: https to the same host is fine", buildable(SVC), "");
  ok("loopback: 127.x is loopback", rh.isLoopbackHost("127.5.5.5") === true);
  ok("loopback: a public address is not", rh.isLoopbackHost("10.200.0.5") === false);
