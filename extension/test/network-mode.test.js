...
(async () => {
  const calls = [];
  const network = { mode: null, effectiveMode: "direct", address: "203.0.113.50", maySwitchMode: true, maySetAddress: false };
  const client = {
  ...
  assert.equal(model.state.vms.rows[0].network.address, "203.0.113.50");
