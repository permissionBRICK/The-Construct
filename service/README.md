...
| Primary lifecycle | `POST /vms`; `POST /vms/{name}/power`; `DELETE /vms/{name}` | Enrolled user for create/power; owner/Admin for delete. Primary delete uses the cascade preview/token flow when children exist; the confirmation may carry `keep: "shared"` to leave shared children in place (a reinstall). |
...
| `Proxmox:Node` / `Storage` / `ImageVolume` / `SnippetStorage` / `SnippetDir` / `Bridge` / `CpuType` / `QmPath` / `PveshPath` | this host / `local-lvm` / `local:import/construct-ubuntu-noble-xfs-amd64.qcow2` / `local` / `/var/lib/vz/snippets` / `vmbr0` / `host` / `qm` / `pvesh` | The Proxmox platform's node, VM-disk storage, cached cloud image, snippet storage and directory, guest bridge, QEMU CPU type and the two commands. Only read with `Backend = proxmox`. |
...
| `Proxmox:ConsolePorts:Start` / `:End` | `5900` / `5999` | Inclusive TCP range for session-bound VNC proxies on `ListenAddress`, advertised through `PublicHost`. Must be within `1-65535` and not overlap SSH or app forwarding ranges. The primary must reach it over a trusted LAN; VNC on this hop is unencrypted. |
...
| `IConsoleTransport`, `IInteractiveConsole` | `ProxmoxConsoleTransport`: bounded native PNG screenshots and keyboard/mouse events through local QMP, with input on stdin. `ProxmoxInteractiveConsole`: one session-bound TCP listener bridged to `qm vncproxy` through binary pipes, with a random password in `LC_PVE_TICKET`. The Construct gateway uses VNC without a Proxmox login; expiry/removal kills the proxy and reconcile reaps revoked or completed sessions. |
...
only `vm-1`; free physical memory was **7,345,060 KiB**. Two attempts to stage
