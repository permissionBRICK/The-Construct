...
public sealed class FakeChildVmDriver(FakeHypervisorDriver hypervisor) : IChildVmDriver
{
    ...
    {
        Check(ct); if (descriptor.Hardware.DynamicMemory is not null) throw new NotSupportedException("Dynamic memory is unsupported.");
        if (!_vms.TryAdd(descriptor.Name, (descriptor, Guid.NewGuid().ToString()))) throw new InvalidOperationException("VM already exists.");
        hypervisor.SetState(descriptor.Name, VmState.Off); Calls.Enqueue("create:" + descriptor.Name); return Task.CompletedTask;
    }
    public Task RemoveAsync(string name, IProgress<string>? progress, CancellationToken ct)
    { Check(ct); _vms.TryRemove(name, out _); hypervisor.SetState(name, VmState.Absent); Calls.Enqueue("remove:" + name); return Task.CompletedTask; }
    public Task UpdateHardwareAsync(string name, ChildHardware hardware, bool resendTemplate, CancellationToken ct)
    {
        ...
        var old = _vms[name]; if (hardware.DynamicMemory is not null) throw new NotSupportedException("Dynamic memory is unsupported.");
        if (old.Descriptor.Hardware.Tpm && resendTemplate) throw new InvalidOperationException("Secure Boot template is locked.");
    ...
    { Check(ct); var d = _vms[name].Descriptor; return Task.FromResult(new VmCapabilitiesSnapshot(name, hypervisor.StateOf(name), true, true, true, false, 1, 1, d.Hardware.Tpm, d.Hardware.Generation, Capabilities.GracefulShutdown, new(CapabilityLevel.Supported, CapabilityLevel.Unsupported, CapabilityLevel.Unsupported))); }
