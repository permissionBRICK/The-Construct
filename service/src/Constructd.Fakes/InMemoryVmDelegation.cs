...
public sealed partial class InMemoryVmRepository
{
    ...
    public Task<IReadOnlyList<Vm>> ListChildrenAsync(string parent, CancellationToken ct)
    {
    ...
    public Task<IReadOnlyList<Vm>> ListSharedAsync(SharingScope scope, CancellationToken ct)
    {
        lock (InMemoryTransaction.Gate)
        {
            return Task.FromResult<IReadOnlyList<Vm>>(
    ...
    public Task<int> CountByOwnerAsync(string owner, VmKind kind, CancellationToken ct)
    {
        lock (InMemoryTransaction.Gate)
        {
    ...
    public Task<IReadOnlyList<Vm>> ListLeasesDueAsync(DateTimeOffset now, TimeSpan retryAfter, CancellationToken ct)
    {
        lock (InMemoryTransaction.Gate)
        {
            return Task.FromResult<IReadOnlyList<Vm>>(
    ...
    public Task<VmOverride?> GetOverrideAsync(string vmName, CancellationToken ct)
    {
        lock (InMemoryTransaction.Gate)
        {
            return Task.FromResult(_overrides.GetValueOrDefault(vmName));
        }
    }
    public Task SetOverrideAsync(VmOverride value, CancellationToken ct)
    {
        lock (InMemoryTransaction.Gate)
        {
            _overrides[value.VmName] = value; return Task.CompletedTask;
        }
    }
    public Task<bool> RemoveOverrideAsync(string vmName, CancellationToken ct)
    {
        lock (InMemoryTransaction.Gate)
        {
            return Task.FromResult(_overrides.Remove(vmName));
        }
    }
    public Task<CascadePreview> SaveCascadePreviewAsync(CascadePreview preview, CancellationToken ct)
    {
        lock (InMemoryTransaction.Gate)
        {
            _cascades[preview.Parent] = preview; return Task.FromResult(preview);
        }
    }
    ...
    public Task<bool> RestoreUnqueuedDeletionAsync(Vm original, CancellationToken ct)
    {
        ct.ThrowIfCancellationRequested();
        lock (InMemoryTransaction.Gate)
        {
            if (original.Deleting || !_vms.TryGetValue(original.Name, out var current) || current.Kind != VmKind.Primary || !current.Deleting ||
                current.ChildCreationClosed || current.CurrentJobId is not null || current.Incarnation != original.Incarnation)
                return Task.FromResult(false);
            _vms[original.Name] = current with { Deleting = false, VmTokenHash = original.VmTokenHash, TokenKind = original.TokenKind };
            return Task.FromResult(true);
        }
    }
}
