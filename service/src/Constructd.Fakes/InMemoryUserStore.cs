...
public sealed partial class InMemoryUserStore : IUserStore, IUserAllowanceStore
{
    ...
    public Task<bool> SetEnabledAsync(string name, bool enabled, CancellationToken ct) { lock (InMemoryTransaction.Gate) { return Change(name, u => u with { Enabled = enabled }, ct); } }
    public Task<bool> SetAllowanceAsync(string name, UserAllowance allowance, CancellationToken ct) { lock (InMemoryTransaction.Gate) { return Change(name, u => u with { Allowance = allowance }, ct); } }
