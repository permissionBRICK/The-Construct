...
{
    private readonly Dictionary<(string Vm, string Incarnation, string Tool, string Day), TokenUsageRow> tokenUsage = [];
    public Task<bool> UpsertAsync(Vm vm, IReadOnlyList<TokenUsageDay> days, DateTimeOffset reportedAt, CancellationToken ct)
    {
        lock (InMemoryTransaction.Gate)
        {
            ...
            var identity = vm.Incarnation ?? vm.Created.ToString("O", System.Globalization.CultureInfo.InvariantCulture);
            foreach (var day in days) tokenUsage[(vm.Name.ToUpperInvariant(), identity, day.Tool, day.Day)] = new(vm.Name, vm.Owner, day, reportedAt, Incarnation: identity);
