...
public sealed record MediaItemResponse(string Id,string Owner,string Name,MediaRole Role,MediaSource Source,string? SourceUrl,
    ...
    DateTimeOffset Created,DateTimeOffset? ReadyAt,int References,bool Shared = false,WindowsMedia? Windows = null)
{
    public static MediaItemResponse From(MediaItem item,int references) => new(item.Id,item.Owner,item.Name,item.Role,item.Source,item.SourceUrl,item.State,item.SizeBytes,item.ReservedBytes,item.Sha256,item.ExpectedSha256,item.Error,item.JobId,item.DedicatedTo,item.Created,item.ReadyAt,references,item.Shared,item.Windows);
}
