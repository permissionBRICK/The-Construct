...
public sealed class ProxmoxDriverTests
{
    ...
    public async Task Network_configuration_updates_cloud_init_only_while_off(bool fixedAddress)
    {
        ...
        await driver.ConfigureNetworkAsync("work-vm", fixedAddress ? "10.0.3.50/22" : null,
            fixedAddress ? "10.0.0.1" : null, fixedAddress ? ["10.0.0.2", "10.0.0.3"] : null, default);
        Assert.Equal(fixedAddress
            ? new[] { "set", "104", "--ipconfig0", "ip=10.0.3.50/22,gw=10.0.0.1", "--nameserver", "10.0.0.2 10.0.0.3" }
    ...
    public async Task Endpoint_is_the_guest_agents_first_real_ipv4_or_null_while_the_agent_is_silent()
    {
        var interfaces = """
            {"result":[
              ...
              {"name":"eth0","ip-addresses":[{"ip-address":"fe80::1","ip-address-type":"ipv6"},{"ip-address":"10.0.3.201","ip-address-type":"ipv4"}]}
            ]}
            """;
        var (driver, runner, _) = Driver(new RecordingProcessRunner()
        ...
        Assert.Equal(new Endpoint("10.0.3.201", 22), endpoint);
