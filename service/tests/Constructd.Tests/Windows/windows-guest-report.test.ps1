...
try {
    ...
    function Get-CimInstance {
        ...
        else { [pscustomobject]@{ PartialProductKey = $global:WindowsReportTest_partial; LicenseStatus = $global:WindowsReportTest_licensed } }
    }
    function Get-ItemProperty {
        param($Path, $ErrorAction)
        if ($Path -like '*CurrentVersion') { [pscustomobject]@{ EditionID = $global:WindowsReportTest_edition; InstallationType = 'Client' } }
        else { [pscustomobject]@{ 'Construct.WindowsKey' = $global:WindowsReportTest_key } }
    }
    ...
    $global:WindowsReportTest_edition = 'Professional'
    $proxmoxPath = [IO.Path]::GetFullPath("$temp/provision/construct-report.ps1")
    ...
    Check ($global:WindowsReportTest_registered.Count -eq 1 -and $global:WindowsReportTest_calls.Count -eq 6) 'Proxmox leaves activation to the host guest-agent channel'
