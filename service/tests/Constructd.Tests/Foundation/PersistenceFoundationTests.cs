...
public sealed class PersistenceFoundationTests : IDisposable
{
    ...
    public async Task PreFeatureDatabaseKeepsRowsAndHashesAndMigrationIsIdempotent()
    {
        ...
        { query.CommandText = $"SELECT COUNT(*) FROM {table}"; Assert.Equal(table == "schema_migrations" ? (long)SqliteMigrations.All.Count : 1L, query.ExecuteScalar()); }
    ...
    public void MigrationRegistryIsOrderedUniqueAndInItsReservedRange()
    {
        ...
        Assert.Equal(ids.Order(), ids); Assert.Equal(ids.Length, ids.Distinct().Count()); Assert.All(ids, id => Assert.InRange(id, 100, 299));
        Assert.Equal(200, SqliteMigrations.SchemaVersion); Assert.Equal(0, SqliteMigrations.MinReadableBy);
    }
