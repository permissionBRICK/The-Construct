...
public sealed class PersistenceFoundationTests : IDisposable
{
    ...
    public void MigrationRegistryIsOrderedUniqueAndInItsReservedRange()
    {
        ...
        Assert.Equal(ids.Order(), ids); Assert.Equal(ids.Length, ids.Distinct().Count()); Assert.All(ids, id => Assert.InRange(id, 100, 699));
        Assert.Equal(600, SqliteMigrations.SchemaVersion); Assert.Equal(0, SqliteMigrations.MinReadableBy);
    }
