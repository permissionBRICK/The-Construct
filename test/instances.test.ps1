...
foreach ($good in @(
    ...
    '{ "backend": "hyperv-remote", "sshHost": "203.0.113.7" }',
...
$byoWarn = @()
$byo = Resolve-ConstructInstanceTarget -Name 'nope-vm' -Path $targetPath -Explicit @{ VmHost = '203.0.113.9' } -WarningVariable byoWarn -WarningAction SilentlyContinue
ok "unknown+endpoint: resolves to the stated host"       ($byo.VmHost -ceq '203.0.113.9')
...
$byoFull = Resolve-ConstructInstanceTarget -Name 'nope-vm' -Path $targetPath -Explicit @{
    VmHost = '203.0.113.9'; HostAlias = 'nope-vm'; SshPort = 2222; LocalKeyName = 'construct_nope-vm_ed25519'; ServiceUrl = 'https://svc.example:7462' } -WarningAction SilentlyContinue
