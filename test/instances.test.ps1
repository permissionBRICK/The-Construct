...
$byoErr = ""
try { Resolve-ConstructInstanceTarget -Name 'nope-vm' -Path $targetPath -Explicit @{ VmHost = '203.0.113.9' } | Out-Null }
...
try {
    Resolve-ConstructInstanceTarget -Name 'nope-vm' -Path $targetPath -Explicit @{
        VmHost = '203.0.113.9'; HostAlias = 'nope-vm'; SshPort = 2222; LocalKeyName = 'construct_nope-vm_ed25519' } | Out-Null
