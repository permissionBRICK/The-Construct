...
CFG
printf '{"sshHost":"10.0.3.50","sshPort":22,"publicHost":"10.0.3.50"}' >"${work}/response"
bash "${repo}/bin/construct-endpoint-refresh.sh"
grep -qx 'CONSTRUCT_EXTERNAL_HOST=10.0.3.50' "${CONFIG_FILE}"
grep -qx 'CONSTRUCT_EXTERNAL_SSH_PORT=22' "${CONFIG_FILE}"
grep -qx "KEEP='untouched value'" "${CONFIG_FILE}"
grep -qx '10.0.3.50' "${work}/renders"
...
CONSTRUCT_AI_TOOLS_FUNCS_ONLY=true REPO_DIR="${repo}" CONSTRUCT_EXTERNAL_HOST=10.0.3.50 \
