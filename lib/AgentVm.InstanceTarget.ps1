...
function Get-ConstructSshFamilyOptions {
    <#
        ...
        a link-local address. With ConnectTimeout set (every ssh call here sets it),
        Windows OpenSSH connects non-blocking and misreads the unreachable first address
        as "Connection established" -- the refusal only surfaces on the first write -- so
        it never tries the next address; only a connect that fails outright falls back.
        This hits any client whose resolver returns an unreachable address first.
        Callers pass -Ipv4 for a VM on a host service. Pure; call as @(...).
    #>
