...
3. **On the domain controller** (optional, for sign-in with Windows accounts), as a domain admin
   ...
   ```sh
   ssh root@test-proxmox.dc.htl-sky.net /opt/construct/host/Constructd.Api admin users add 'HOME\alice' --max-vms 3
   ```
4. **On a PC** with The Construct installed, the normal remote command, with your Windows account:

   ```powershell
   .\Auto-Install.ps1 -Backend hyperv-remote -ServiceUrl https://test-proxmox.dc.htl-sky.net:7462 -InstanceName work-vm
   ```
...
name** (`test-proxmox.dc.htl-sky.net` below). Kerberos tickets are issued for a name, never for an
address.
...
1. **On a domain controller**, as a domain admin (needs RSAT's ActiveDirectory and DnsServer modules and `ktpass`):
   ...
   ```powershell
   .\service\host\New-ConstructKerberosPrincipal.ps1 -HostFqdn test-proxmox.dc.htl-sky.net -Address 10.0.3.184 -InstallOnHost root@test-proxmox.dc.htl-sky.net
   ```
...
2. **On the node** (what `-InstallOnHost` does for you), with the host's DNS name as the public
   ...
   ```sh
   bash service/host/install-construct-host.sh --public-host test-proxmox.dc.htl-sky.net \
        --keytab /root/constructd.keytab --netbios-domain HOME --realm DC.HTL-SKY.NET
   ```
...
3. **On the node**, add people by their domain name, then they enrol without a token:
   ...
   ```sh
   /opt/construct/host/Constructd.Api admin users add 'HOME\christoph' --max-vms 3
   ```
...
|---|---|---|
| Host FQDN | `test-proxmox.dc.htl-sky.net` | DNS record, SPN, certificate, client URL |
| Realm (the AD domain, upper case) | `DC.HTL-SKY.NET` | principal name, `krb5.conf` |
| NetBIOS domain | `HOME` | how the service names users (`HOME\alice`) |
| Service account | `svc-constructd` | owns the SPN and the keytab |
| Principal | `HTTP/test-proxmox.dc.htl-sky.net@DC.HTL-SKY.NET` | the identity in the keytab |
...
1. **DNS: make the name resolve to the node.** Skip this when your DNS lives elsewhere (a
   ...
   ```powershell
   Add-DnsServerResourceRecordA -ZoneName dc.htl-sky.net -Name test-proxmox -IPv4Address 10.0.3.184 -TimeToLive (New-TimeSpan -Hours 1)
   Resolve-DnsName test-proxmox.dc.htl-sky.net
   ```
...
3. **Register the SPN.** The service principal name is what a client asks the KDC for when it
   opens `https://test-proxmox.dc.htl-sky.net`. It must exist exactly once in the forest.

   ```powershell
   setspn -Q HTTP/test-proxmox.dc.htl-sky.net          # must find nothing
   setspn -S HTTP/test-proxmox.dc.htl-sky.net svc-constructd
   ```
...
4. **Set the UPN to the principal.** This is the one non-obvious step. For AES keys the key is
   ...
   ```powershell
   Set-ADUser -Identity svc-constructd -UserPrincipalName 'HTTP/test-proxmox.dc.htl-sky.net@DC.HTL-SKY.NET'
   ```
...
5. **Write the keytab.** `ktpass` sets the account password and writes a keytab containing the
   ...
   ```powershell
   ktpass /princ HTTP/test-proxmox.dc.htl-sky.net@DC.HTL-SKY.NET /mapuser HOME\svc-constructd /crypto AES256-SHA1 /ptype KRB5_NT_PRINCIPAL /pass +rndPass /mapop set +setupn +DumpSalt /out C:\temp\constructd.keytab
...
6. **Install the keytab on the node.** Copy it over and run the installer with the Kerberos
   ...
   ```sh
   scp C:\temp\constructd.keytab root@test-proxmox.dc.htl-sky.net:/root/constructd.keytab
   bash service/host/install-construct-host.sh --public-host test-proxmox.dc.htl-sky.net \
        --keytab /root/constructd.keytab --netbios-domain HOME --realm DC.HTL-SKY.NET
   ...
   `Constructd:Negotiate:{Enabled:true, DomainName:"HOME", Realm:"DC.HTL-SKY.NET"}` in
...
8. **Verify** from a domain-joined PC, in the user's own session (not an elevated or system one:
   ...
   ```powershell
   klist get HTTP/test-proxmox.dc.htl-sky.net      # a ticket for the SPN; 0xc000018b = SPN not found or account disabled
   curl.exe --negotiate -u : https://test-proxmox.dc.htl-sky.net:7462/api/v1/whoami
   ```
...
Kerberos names the user `christoph@DC.HTL-SKY.NET`; the service maps that onto `HOME\christoph`
