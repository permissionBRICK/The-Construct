...
kept. The log holds what the console showed, and the prompts, but never what was typed at
them. (It is a plain file writer, not a PowerShell transcript: on Windows PowerShell 5.1 a
transcript slows every console line.) A run that ends in an error pauses with "Press Enter
