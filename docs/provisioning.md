...
| `T3CODE_LIMIT_RESUME` | `false` | Legacy-compatible key for the optional patched T3 source build. When true, resolve the channel's exact Git tag, build the VM server and unsigned Windows Desktop installer together, and add voice input, usage-limit recovery, and OpenCode monitoring. Requires about 15 GiB free for the first build; unchanged builds are cached |
