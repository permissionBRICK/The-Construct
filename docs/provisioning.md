...
| `T3CODE_LIMIT_RESUME` | `false` | Legacy-compatible key for the optional patched T3 source build. When true, resolve the channel's exact Git tag and build the VM server and unsigned Windows Desktop installer together. Both gain live cursor-safe voice input (mic button, level ring, and Ctrl+T), Claude usage-limit recovery, and OpenCode monitoring. Requires about 15 GiB free for the first build; unchanged T3/Construct builds are skipped |
