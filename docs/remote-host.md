...
| `Installing browser console gateway` fails with `TLS handshake timeout` / `failed to resolve reference "docker.io/guacamole/guacd…"` | Docker Hub is not reachable from the VM's network (a company network may block it outright — the same PC's browser cannot open docker.io either). The step is optional: provisioning finishes without the browser console and lists the step in its summary. Re-run `sudo bash /opt/construct/repo/console-viewer/install.sh` once the registry is reachable, or configure a mirror in the VM's `/etc/docker/daemon.json` first. |
