...
delegated child VMs and signed host updates on the `pc-1` Hyper-V host. It assumes the
original [remote-host field test](field-test-remote-host.md) passed and the existing primary
`vm-1` must remain usable throughout.
...
| `[FIELD]` | Must be performed on `pc-1`; it was **not** performed while writing these docs. |
...
|---|---|
| Hyper-V host | `pc-1` |
| Existing primary that must survive | `vm-1` |
...
- [ ] `[FIELD]` Keep `vm-1` running and record its Hyper-V state, SSH endpoint, Construct
...
**Stop condition:** do not continue if `vm-1` is already unhealthy, the database cannot be
...
- [ ] `[FIELD]` Stop only `constructd`. Confirm every Hyper-V VM, especially `vm-1`, keeps
...
- [ ] `[FIELD]` When startup succeeds, run:
  ...
  **Expect:** database check succeeds; `/health` advertises `host-admin`, `children`, `media`,
  `console`, `updates` and `network`; `vm-1` is still Running and its SSH/application checks
  still pass.

## 2. Verify migration of `vm-1`
...
- [ ] `[FIELD]` Open **Construct: Host Administration** from the enrolled client. The Overview
      ...
      policy.
- [ ] `[FIELD]` In **VMs**, find `vm-1`. Record that it is `kind=primary`, has no parent,
...
- [ ] `[FIELD]` Connect to `vm-1`, exercise its existing Start/Save/forward/provision path,
      and verify old client enrollment and tokens still work. This is the zero-change default
      path.
- [ ] `[FIELD]` Inside `vm-1`, run `construct vm identity --json` **before** credential
...
- [ ] `[FIELD]` Run `Provision-AgentVM.ps1 -InstanceName vm-1 -RotateVmToken` once
...
- [ ] `[FIELD]` Keep the existing owner as Admin. Register a separate ordinary user for the
      ...
      through the ordinary remote-primary flow. Do not use `vm-1` for later cascade deletion.
...
- [ ] `[FIELD]` Record `activatedAt`, `expiresAt`, service time and the lease scheduler interval.
      ...
      extends `expiresAt`; `vm-1` remains unaffected.
...
- [ ] `[FIELD]` Confirm `vm-1` and at least one disposable child are Running. Record their VM
...
- [ ] `[FIELD]` During handoff/replacement, watch Hyper-V from a separate elevated console.
      **Expect:** `vm-1` and the child never stop, save, pause or reboot. The Admin panel shows
...
- [ ] `[FIELD]` Re-run `admin db check`, compare settings/certificate/database/media/user/token/VM
      inventories with the pre-update record, and exercise `vm-1` SSH and application URLs.
...
- [ ] `[FIELD]` Record approval and the test release commit. Stage and apply it while `vm-1`
...
This is why all destructive children belong to `ha-parent`, not `vm-1`.
...
- [ ] `[FIELD]` Confirm `vm-1`, `ha-consumer`, unrelated shared media and their forwards remain.
...
- [ ] `[FIELD]` Remove remaining disposable media/VMs and restore any temporary allowance or
      host-config changes. Keep `vm-1` enrolled and verify it once more.
