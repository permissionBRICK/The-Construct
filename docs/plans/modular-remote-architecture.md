...
| in flight | Phase 2 review round 8 found 2 must-fix on the B8x wiring (forwarder started at activation on the default path → decided: stays enabled by default but lazy + guest-gated; start/stop generation race) + 1 should-fix (config-branch validators accept a trailing dot) — fix pair on `fix/p2-r8`; B9 docs pass on `docs/b9` (incl. `docs/field-test-remote-host.md` checklist). Remaining: field test on the home domain. |
