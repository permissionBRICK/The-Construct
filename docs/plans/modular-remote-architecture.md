...
| in flight | B8x (extension forwarder module + service ack relay + idle-policy UI); Phase 2 review round 7 found 4 must-fix (remote multi-VM endpoint check in Auto-Install, hyperv-remote vmName canonical identity, manual-vs-auto mic enable race, remote entries without sshHost) — fix pair on `fix/p2-r7`. Remaining: B9 docs + field test on the home domain. |
