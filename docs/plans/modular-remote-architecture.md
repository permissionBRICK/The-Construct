...
> Follow-on design (2026-09-06): [remote host administration and temporary child VMs](host-administration-and-child-vms.md), with a separate [implementation plan](host-administration-implementation.md). The scope is implemented and automated on Linux fakes as of 2026-09-07, but has not been field-validated on Hyper-V; see the status lines and [field checklist](../field-test-host-admin.md).
...
**Field finding (2026-09-02, `pc-1`, WSL 2.6.3):** `wsl.exe` as LocalSystem exits -1 with
...
Decided while preparing the first pc-1 remote VM; each is a batch of its own (Phase 7),
none blocks Phase 6.
...
| Host stays awake while VMs run (**B18**) | The pc-1 slept overnight with VMs expected to run. `constructd` holds a Windows power availability request (`PowerCreateRequest`/`PowerSetRequest`, SystemRequired) while any VM it manages is running and releases it when none is; installer prints the host's sleep timeouts and offers to set AC standby/hibernate to never. Until then the field-prep set the pc-1 timeouts by hand (2026-09-04). |
...
**First remote VM from a client PC (2026-09-04 evening, pc-1/pc-1 + main PC):** the run
...
| 2026-09-02 | Field test started on `pc-1` (WSL 2.6.3, German Windows). Installer fixes from the field, each direct + tested: relative path params resolved against `$PWD` before the elevated relaunch and ACE identities read by SID (`6ceaf98`); parents hardened before children (`8aa8ca6`); LocalSystem task polled via the Schedule.Service COM API in its own folder, `SCHED_S_TASK_RUNNING` is not an exit code (`16405cc`); failed LocalSystem commands report their output (`b56e0e9`). Blocker: `WSL_E_LOCAL_SYSTEM_NOT_SUPPORTED` → §4.10 / B10. |
...
| 2026-09-04 | **First remote VM in the field**: `vm-1` created on the pc-1 host service and provisioned from the main PC (Kerberos, pre-built ISO, 21 profiles, T3 + OpenCode reachable through service host forwards :2300/:2301, CA trusted, Desktop installed). Fixes landed on main during the run: PS 5.1 pin validator (d76591e), driver-load clobber (4f9f96b, 466cbd1), unattended clone credentials (84c64d1), pre-built ISO seed user + sudo + DHCP reconfigure (c54e732), service certificate handed to the guest (3fb43e5). Interim on the host: per-minute forward-sync task, OpenVPN + jarvis relay as boot tasks, Defender exclusions, DNS registration limited to the tunnel adapter. Open: B19, service redeploy, gitgudlab `slidesaver` upload-pack hang. |
| 2026-09-04 | Field follow-ups from the first remote VM (vm-1): (1) the installers now RECORD the VM size they create (`vmMemoryGB`/`vmDiskGB`/`vmCpuCount` via `Save-ConstructVmSpec`, both Auto-Install paths + a hand-run Create-AgentVM) and the panel's VM-resources fields no longer carry fabricated defaults (placeholders from the live VM; an empty field keeps the VM's size; `-VmCpuCount` rides rebuilds); (2) the extension backfills, once, an instance's missing size + keep-saved config (`t3code`, `t3codeChannel`, `t3codeLimitResume`, `opencodeBackgroundWatcher`) from the probe, so a console-installed VM's settings form is honest (`instancestate.backfillFromProbe`); (3) `Update-T3Code.ps1` forwards the keep-saved toggles only when saved instead of assuming `T3CodeLimitResume` TRUE — the cause of a stock remote VM turning into a patched source build on the Desktop's update; (4) the Desktop install rule compares the manifest's `patchHash` (falling back to `buildHash`), so a Construct commit bump no longer reinstalls a byte-identical Desktop and two VMs at different commits no longer take turns doing so. Not changed: linking a VM in T3 Desktop remains the manual pairing step (panel → instance → T3 ▷, then Desktop Settings → Connections). |
| 2026-09-05 | **T3 Desktop auto-link** (§4.12 "T3 Desktop topology", decided with the project owner): the Desktop app links every registry instance with a T3 server that no linked remote matches -- automatically on its poll, the default local VM included -- via the new non-interactive `Get-ConstructT3PairingLink.ps1` (SSH-minted one-time link, `--scopes administrative` on patched builds through a new `t3 auth pairing create --scopes` transform), the app's own connect command, and a per-instance `t3Link` marker (once; 30-min backoff after a failure; a connection removed by hand is never re-added -- removal stays manual). Providers rows for unlinked instances carry a manual **Link**. The VM-side build cache and the Desktop install rule are keyed by T3 version + patch recipe (the Construct commit no longer rebuilds/reinstalls); a stock T3 reinstall is skipped when the release is unchanged. test/t3-overlay.test.sh now resolves transformed workspace packages from the worktree, so contract changes are typechecked. |
| 2026-09-07 | Host administration and child-VM phases 0–6 implemented: migrated primary inventory, users/allowances, capacity/media, general-purpose child lifecycle, sharing/expiry/cascade, extension admin + user views, network/console adapters and signed drain/update/rollback. Linux validation baseline: 1,178 .NET tests plus the Node, PowerShell and bash contract suites; no production Hyper-V validation. Operator docs and `docs/field-test-host-admin.md` record the rollout and remaining capability limits. |
| next | Field test resumes on `pc-1`: install with the pre-built ISO, verify `admin iso status`, first VM's hostname + `<name>.mshome.net`. |
