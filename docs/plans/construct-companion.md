...
- Windows: `--selftest` is the field entry point. If the Jarvis relay to the Windows host
  answers (`node /root/repos/jarvis/scripts/jarvis-relay.mjs run --target pc-1 --shell powershell --timeout 300 -- '<cmd>'`,
...
- Reset `cc/integ-4` to `feat/companion` (`bbec973`), then merged `cc/s3-integration` (`adea0e4`) with `--no-ff` as `8d6915d`. No conflicts or skipped branches; no integration code repair or new design deviation. S3 production composition, runtime/settings wiring, installer entrypoints and the owner amendment are preserved. Full per-suite counts and scope: [stage 4 integration results](construct-companion-stage-4-integration.md).
- Linux validation: Companion build 0 warnings/0 errors; Companion tests 4,725/4,725; service tests 1,261/1,261; Node 33/33 suites; PowerShell 30/30 suites (24 under `test/`, six recursively under `service/tests/`) after the existing config-sync environment retry; Bash 20/22 suites. Companion installer/entrypoint/package assertions: 75/33/34. Two optional checks skipped (VS Code proxy agent and Windows DPAPI).
- PowerShell config-sync initially passed 561/568, then 568/568 with process-local `init.defaultBranch=main` and the six documented `CONSTRUCT_*` variables unset. No global/repository Git configuration changed. Node configsync passed 475/475 without a retry.
- Existing Bash defects remain: `contracts-compile.test.sh` 4/5 (`HypervisorVmInfo` frozen signature mismatch), `idle-report.test.sh` 100/103 (service-key count/URL/instance). Affected tests, contract document and service/guest code are unchanged from the starting ref; failures match prior integration baseline evidence. No merge-induced regression was found.
- Regenerated 4,456 parity rows across 31 areas twice with zero fixture drift. .NET build/test invocations were serial; all test runtime processes exited. No existing VM service was stopped or restarted. No Windows build, selftest, installation or native runtime/UI execution was performed.
- Inherited unsupported workflows remain in `companion/README.md`: attached-window registration/conversion, project creation, removal, first-VM wizard, install-wide update orchestration, one-time token display, automatic checkpoint apply, lifecycle preflight/live-project fallback and result monitoring. These are documented implementation gaps beyond a few-line merge repair; Windows field validation also remains pending. The S3 implementation supersedes the older input-merge notes about missing production composition and D11 entrypoints.

## Integration notes (stage 5)

- Reset `cc/integ-5` to `feat/companion` (`e044b47`), then merged all five `cc/s4-cleanup` commits through `2e187bd` with `--no-ff` as `8699604`. No skipped branches or conflicts; no integration code fix or new design deviation. Cleanup intent and all recorded S4 deviations are preserved. Full per-suite counts: [stage 5 integration results](construct-companion-stage-5-integration.md).
- Linux validation: Companion build 0 warnings/0 errors; Companion 4,749/4,749; service 1,261/1,261; Node 33/33 suites on the first attempt; PowerShell 30/30 suites after the existing config-sync environment retry (24 under `test/`, six recursively under `service/tests/`); Bash 20/22 suites. Companion entrypoint/installer/package checks: 33/72/34. Optional VS Code proxy-agent and Windows DPAPI checks skipped.
- PowerShell config-sync initially passed 561/568, then 568/568 with the six documented `CONSTRUCT_*` variables unset and process-local `init.defaultBranch=main`. Node configsync passed 475/475 initially. No global/repository Git configuration changed.
