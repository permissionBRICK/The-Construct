...
Date: 2026-09-07 (revision 6 after review). Branch: `ha/s0-contracts`.
...
only.** Inside the per-VM `MutateAsync`, before any row below releases a reservation,
promotes one, marks a lease overdue or bumps `power_generation`, reconciliation (a) reads
the VM row through `IAdmissionScope.ReadVmAsync` and skips the VM when its
`PowerGeneration` or `CurrentJobId` differs from the value read during the snapshot pass
(an operation ran in between; the next pass re-observes), and (b) re-reads the VM's state
from the hypervisor under the held gate (`IHypervisorDriver.GetStateAsync`, a single
per-VM call) and acts on **that** fresh read, never on the pre-gate snapshot. A
`GetStateAsync` failure ⇒ treat as `Unknown` (keep everything). S3 pins the interleaving
"snapshot says Off → VM starts and confirms → reconciliation acquires the gate" (nothing
released, generation untouched).
...
|---|---|
| Create with `start=true` | request stored; `pending` reservation; on confirmed `Running`: `ActivatedAt = activationBase`, `ExpiresAt = activationBase + requested` (or `unlimited`), where **`activationBase` is the moment the start was accepted** (the intent's `acceptedAt`, §7.3), not the later observed `Running`. **Decided here (one rule for first starts and retries):** activation from acceptance is conservative by the driver-call latency (seconds) on the normal path and makes a delayed retry unable to extend the lease; §5.3a and §7.3 use the same base. |
...
| `update-not-commitable` | 409 | `reason` |
...
**Precedence:** a fence for the handoff's `updateId` is evaluated **before** the record's
outcome (a `commitOnly` or `closed` fence written by a resolution after an earlier
...
| present | any | fence `rollbackAuthorized` | any | stay in `maintenance` until the rollback outcome is recorded |
...
`handoff.commit` **and** the service passes its own health gate (migrated schema, `db
check` clean); `abort`: `RecoveryRecord.BackupComplete`; `close`: the running binary's
