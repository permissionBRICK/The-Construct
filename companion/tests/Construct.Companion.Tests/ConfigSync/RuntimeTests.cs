...
public sealed class RuntimeTests
{
    [Fact] public async Task PerInstanceThrottleAndWireState()
    {
        ...
        await using var a=new ConfigSyncRuntime(w.Engine,w.Repo,remotes,w.Clock,new(1,1));
        Assert.Null((await a.BuildStateAsync()).LastSyncAt); Assert.NotNull(await a.TickAutoAsync()); Assert.Null(await a.TickAutoAsync());
        await using var b=new ConfigSyncRuntime(new(w.Repo,w.Lock,w.Ssh,"vm-b",w.Store),w.Repo,remotes,w.Clock,new(1,1)); Assert.True(b.DueForAuto); Assert.Null((await b.BuildStateAsync()).LastResult);
    ...
    {
        using var w=new GitTestWorkspace(); w.Host("a"); var remotes=new ConfigRemotes(w.Repo,w.Cache); await using var runtime=new ConfigSyncRuntime(w.Engine,w.Repo,remotes,w.Clock,new(1,1));
    ...
    {
        using var w=new GitTestWorkspace(); w.Host("a"); var remotes=new ConfigRemotes(w.Repo,w.Cache); var runtime=new ConfigSyncRuntime(w.Engine,w.Repo,remotes,w.Clock,new(1,1)); await runtime.SyncNowAsync(); runtime.StartWatching(); var before=w.Ssh.Calls;
    ...
    {
        using var w=new GitTestWorkspace(); w.Host("a"); var runner=new RejectConfigRunner(w.Processes); var repo=new ConfigRepository(new(runner),w.Files,w.Repo.Directory); var engine=new ConfigSyncEngine(repo,w.Lock,w.Ssh,storeRoot:w.Store);
    ...
    [Fact] public async Task FactoryQueuesDifferentInstancesBehindActiveTick()
    {
        ...
        var factory=new ConfigSyncFactory(w.Files,new HostProcesses(),w.Processes,w.Clock,new FakePrompts(),new FakeClipboard());
