...
public sealed class NetworkLockTests
{
    [Fact] public async Task BlockedImportCloneDoesNotHoldProvisioningLock()
    {
        ...
        var runner=new BlockingNetworkRunner(w.Processes); var repo=new ConfigRepository(new(runner),w.Files,w.Repo.Directory); var remote=new ConfigRemotes(repo,w.Cache); remote.WriteRemotes([new(url)]);
        var actions=new ConfigSyncActions(repo,remote,w.Lock,new FakePrompts(),new FakeClipboard(),w.Clock,new(1,1)); var import=actions.ImportAsync();
        await runner.Entered.Task.WaitAsync(TimeSpan.FromSeconds(5)); var other=new SyncLock(w.Files,new HostProcesses(),w.Clock,repo.Directory); string? token=null;
