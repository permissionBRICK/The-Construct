...
public sealed class HostAdminSecretsTests
{
    ...
    public async Task IssueTokenAcceptsLegacyFieldsAndReportsMissingPlaintext(string field)
    {
        ...
        var request=Assert.Single(api.Requests,r=>r.Method=="POST");Assert.Equal("issued from VS Code",request.Body!.Value.GetProperty("label").GetString());
    ...
    public async Task SecretGoesOnlyToNativePromptAndCancelledRotationDoesNothing(string action,bool accept)
    {
        ...
        Assert.Equal(accept?1:0,api.Requests.Count(r=>r.Method=="POST"));
        if(accept)
        {
            ...
            var request=Assert.Single(api.Requests,r=>r.Method=="POST");Assert.Equal(action=="issueToken"?"/api/v1/users/alice/tokens":"/api/v1/vms/alice/token",request.Url.AbsolutePath);
