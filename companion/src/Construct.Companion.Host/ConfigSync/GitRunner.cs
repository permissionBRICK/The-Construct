...
public sealed class GitRunner(IProcessRunner processes, string command = "git")
{
    ...
    public async Task<ProcessResult> RunAsync(string cwd, IEnumerable<string> args, bool identity = false, string? index = null, bool network = false, CancellationToken cancellationToken = default)
    {
        ...
        try
        {
            var result = await processes.RunAsync(new(command, argv, cwd, Timeout: TimeSpan.FromSeconds(network ? 60 : 30)) { EnvironmentOverrides = index == null ? null : new Dictionary<string, string?> { ["GIT_INDEX_FILE"] = index } }, cancellationToken);
        ...
        catch (Exception) { return new(-1, Stderr: "git process failed"); }
    }
