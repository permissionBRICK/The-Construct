...
describe("ConstructUpdates target VM (instances.json)", () => {
  ...
  it("derives a non-default default instance like instances.js and passes its identity", () => {
    ...
    assert.equal(target.vmHost, "203.0.113.7");
  ...
  it("refuses what the panel's registry rules refuse, naming the reason", () => {
    ...
    assert.include(
      problemOf({
        defaultInstance: "work",
        instances: { work: { backend: "hyperv-remote", sshHost: "203.0.113.7", vmName: "Other" } },
      }),
      "vmName",
    );
    assert.include(
    ...
    assert.include(
      problemOf({
        defaultInstance: "work",
        instances: { work: { backend: "hyperv-remote", sshHost: "203.0.113.7", hostAlias: "a b" } },
      }),
      "unusable hostAlias",
    );
    assert.include(
      problemOf({
        defaultInstance: "work",
        instances: { work: { backend: "hyperv-remote", sshHost: "203.0.113.7", keyName: "../x" } },
      }),
      "unusable keyName",
    );
    assert.include(
      problemOf({
        defaultInstance: "work",
        instances: { work: { backend: "hyperv-remote", sshHost: "203.0.113.7", keyName: "CON.key" } },
      }),
      "unusable keyName",
    );
    ...
    assert.include(
      problemOf({
        defaultInstance: "work",
        instances: {
          work: { backend: "hyperv-remote", sshHost: "203.0.113.7" },
          other: { backend: "hyperv-remote", sshHost: "203.0.113.7" },
        },
      }),
      "share the endpoint",
    );
    assert.include(
      problemOf({
        ...
        instances: {
          work: { backend: "hyperv-remote", sshHost: "203.0.113.7" },
          other: { backend: "hyperv-remote", sshHost: "203.0.113.8", hostAlias: "WORK" },
        },
      }),
      "share the ssh alias",
    );
    ...
    assert.include(
      problemOf({
        defaultInstance: "work",
        instances: { work: { backend: "Hyperv-Remote", sshHost: "203.0.113.7" } },
      }),
      "not spelled",
    );
    assert.include(
    ...
    for (const host of ["203.0.113.7", "vm.corp.example", "2001:db8::7", "::ffff:203.0.113.7"]) {
      assert.isNull(
        readConstructVmTarget({
          defaultInstance: "work",
          instances: { work: { backend: "hyperv-remote", sshHost: host } },
    ...
    assert.equal(
      readConstructVmTarget({
        defaultInstance: "work",
        instances: { work: { backend: "hyperv-remote", sshHost: "203.0.113.7", sshPort: "70000" } },
      }).sshPort,
      22,
    );
    ...
    assert.isNull(
      readConstructVmTarget({
        ...
        instances: {
          work: { backend: "hyperv-remote", sshHost: "203.0.113.7" },
          broken: { backend: 42, sshHost: "203.0.113.7" },
          alias: {
            backend: "hyperv-remote",
            sshHost: "203.0.113.9",
            hostAlias: "work",
    ...
    assert.include(
      problemOf({
        ...
        instances: {
          work: { backend: "hyperv-remote", sshHost: "203.0.113.7" },
          legacy: { backend: "proxmox", sshHost: "203.0.113.7" },
        },
      }),
      "share the endpoint",
    );
    assert.include(
      problemOf({
        ...
        instances: { legacy: { backend: "proxmox", sshHost: "203.0.113.7" } },
      }),
      "unknown backend",
    );
